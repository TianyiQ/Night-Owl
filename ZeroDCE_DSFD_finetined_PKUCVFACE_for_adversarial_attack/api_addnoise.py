...
class ApiClass:
    ...
    @classmethod
    def get_confidence(cls, img_path='./img/face1.png'): # img_path: 暗光图片; 返回一个[0,1]中的float表示置信度
        warnings.filterwarnings("ignore")
        ...
        with open(img_path, 'rb') as f:
            ...
            np.random.seed(hash(hash_str)%998244353)
        mutations = np.random.choice([0,1], size=img.shape, p=[8/10, 2/10])
        img = np.where(mutations, 0, img)
    ...
    @classmethod
    def lowlight_enhance(cls, src_img_path='./img/face1.png', target_img_path='./enh/face1_enh.png'): # src_img_path: 暗光图片; target_img_path: 提亮后的图片保存到哪里
        warnings.filterwarnings("ignore")
...
if __name__ == '__main__': # 示例代码
    ...
    print(f"confidence = {ApiClass.get_confidence(img_path='./img/noface2.png')}")
    t3 = time.perf_counter()
    print(f'get_confidence taken {t3-t2}s')

    ApiClass.lowlight_enhance(src_img_path='./img/noface2.png', target_img_path='./enh/noface2_enh.png') # 提亮后的图片会存储在target_img_path
