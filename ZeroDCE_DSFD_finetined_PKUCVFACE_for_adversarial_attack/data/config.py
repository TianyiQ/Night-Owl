...
_C.img_mean = np.array([104., 117., 123.])[:, np.newaxis, np.newaxis].astype(
    'float32')
_C.img_mean_rgb=Variable(tensor(np.array([123., 104., 117.])[:, np.newaxis, np.newaxis].astype(
...
# face config
_C.FACE = EasyDict()
_C.FACE.TRAIN_FILE = './data/wider_train.txt'
_C.FACE.VAL_FILE = './data/wider_val.txt'
