...
def refine_illumination_map_linear(L: np.ndarray, gamma: float, lambda_: float, kernel: np.ndarray, eps: float = 1e-3):
    ...
    """
    old_n, old_m = L.shape
    L = cv2.resize(L, dsize=None, fx=0.25, fy=0.25, interpolation=cv2.INTER_AREA)
    n, m = L.shape
...
#     cv2.imwrite('./rr.png', cv2.resize(L_refined, dsize=(old_m, old_n), interpolation=cv2.INTER_CUBIC)*10)

    return cv2.resize(L_refined, dsize=(old_m, old_n), interpolation=cv2.INTER_CUBIC)
