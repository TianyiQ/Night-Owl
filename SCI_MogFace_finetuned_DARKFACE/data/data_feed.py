...
class TrainFeed(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.add_dsfd_label:
            # img, bbox_labels, anchors, dsfd_bbox_labels = self.dataset[index]
            # return torch.from_numpy(img).permute(2, 0, 1), torch.from_numpy(bbox_labels), torch.from_numpy(anchors), dsfd_bbox_labels
