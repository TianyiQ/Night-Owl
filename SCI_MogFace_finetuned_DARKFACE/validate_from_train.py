...
if __name__ == '__main__':
    ...
    log_file_name = os.path.join(log_dir, f'val{args.resume_iter}_log.txt')
    ...
    # train data feed
    dataset = create(cfg.train_feed_val)
    epoch_size = len(dataset) // args.batch_size

    if 'collate_fn' in cfg:
        collate_fn = create(cfg.collate_fn)
        data_loader = data.DataLoader(dataset, args.batch_size, num_workers=args.num_workers,
                                      shuffle=False, pin_memory=True, collate_fn=collate_fn) # shuffle=False
    else:
        data_loader = data.DataLoader(dataset, args.batch_size, num_workers=args.num_workers,
                                      shuffle=False, pin_memory=True) # shuffle=False
    ...
    # optimizer
    use_warmup = False # 'warmup_strategy' in cfg
    # if use_warmup:
    ...
    loc_loss_all = []
    total_loss_all = []
    for iter_idx in tqdm(range(epoch_size)): # changed
        t1 = time.time()
        with torch.no_grad():
            net.eval()
            # if use_warmup:
            ...
            try:
                ...
                if use_ali_ams:
                    ...
                    tp = next(batch_iterator)
                    if len(tp) == 4:
                        images, targets, anchors, bbox_labels_list = tp
                    elif len(tp) == 3:
                        images, targets, bbox_labels_list = tp
                    else:
                        images, targets = tp
            except StopIteration:
                cur_epoch_num += 1
                if iter_idx < epoch_size-1:
                    print(f'\nnew epoch: {cur_epoch_num}', end='\n\n')
                batch_iterator = iter(data_loader)
            ...
            if use_ali_ams:
                ...
                tp = parallel_net(images)
                if len(tp)==2:
                    cls, loc = parallel_net(images)
                else:
                    cls, loc, _, __ = parallel_net(images)
                loss = criterion(cls, loc, targets)
                # loss = criterion(cls, loc, targets, anchors, bbox_labels_list)
                cls_loss, loc_loss, total_loss = loss
            ...
            cls_loss_all.append(cls_loss.data)
            loc_loss_all.append(loc_loss.data)
            total_loss_all.append(total_loss.data)
...
avg_cls_loss = sum(cls_loss_all)/len(cls_loss_all)
avg_loc_loss = sum(loc_loss_all)/len(loc_loss_all)
avg_total_loss = sum(total_loss_all)/len(total_loss_all)

print(f'validated iter{args.resume_iter}:\n\tcls_loss = {"%.8f"%avg_cls_loss}\n\tloc_loss = {"%.5f"%avg_loc_loss}\n\ttot_loss = {"%.5f"%avg_total_loss}')