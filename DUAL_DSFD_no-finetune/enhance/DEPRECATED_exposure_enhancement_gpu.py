...
# project
from enhance.utils import get_sparse_neighbor
...
def compute_smoothness_weights(L: np.ndarray, x: int, kernel: np.ndarray, eps: float = 1e-3):
    ...
    """
    # Lp = np.array(cv2.Sobel(L.get(), cv2.CV_64F, int(x == 1), int(x == 0), ksize=1))
...
def fuse_multi_exposure_images(im: np.ndarray, under_ex: np.ndarray, over_ex: np.ndarray,
    ...
    # images = [np.clip(x * 255, 0, 255).astype("uint8").get() for x in [im, under_ex, over_ex]]
...
def refine_illumination_map_linear(L: np.ndarray, gamma: float, lambda_: float, kernel: np.ndarray, eps: float = 1e-3):
    ...
    point1_time = perf_counter()
    print(f'len={len(row)}')
    print(f'phase1 {point1_time-start_time} sec')
    F = cp_csr_matrix((cp.array(data), (cp.array(row), cp.array(column))), shape=(n * m, n * m))
    point2_time = perf_counter()
    print(f'phase2 {point2_time-point1_time} sec')

    # solve the linear system
    Id = cp_diags([np.ones(n * m)], [0])
    A = Id + lambda_ * F
    L_refined = cp_spsolve(cp_csr_matrix(A), cp.array(L_1d)).reshape((n, m)).get()
    point3_time = perf_counter()
    print(f'phase3 {point3_time-point2_time} sec')
...
def enhance_image_exposure(im, gamma: float, lambda_: float, dual: bool = True, sigma: int = 3,
    ...
    # return np.clip(im_corrected * 255, 0, 255).astype("uint8").get()
