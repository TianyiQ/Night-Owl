...
class GetFileSuffix:
    ...
    def get_dict(cls):
        if cls.suf_dict:
            return
        path = '/code/dataset/WIDERFACE/WIDER_train/images/'
        for path, dir_list, file_list in os.walk(path):
            for file_name in file_list:
                cls.suf_dict[file_name.split('.')[0]] = file_name.split('.')[1]
        print(f"finish suf_dict precalc: suf_dict['0010']={cls.suf_dict['0010']}")
    
    @classmethod
    def get_suf(cls, file_name):
        cls.get_dict()
        assert file_name in cls.suf_dict
        return '.' + cls.suf_dict[file_name]
...
class WiderFaceValSet(object):
    ...
    def __next__(self):
        ...
        abs_img_path = os.path.join(self.img_dir_name, item[1], item[0]) + GetFileSuffix.get_suf(item[0]) # '.png'
    ...
    def parse_gt_mat(self):
        ...
        for index, event in enumerate(event_list):
            ...
            for num, file in enumerate(filelist):
                ...
                ret_list.append([im_name.replace(' ',''), dir_name.replace(' ','')])
...
class WiderFaceTrainSet(object):
    ...
    def __getitem__(self, gt_idx):
        ...
        if self.out_dsfd_label:
            ...
            # return img, encode_bbox_labels, self.anchors, encode_dsfd_bbox_labels
        elif self.out_bbox_anchor:
...
@register
class WiderFaceTrainSet_Val(WiderFaceTrainSet):
    '''
    TODO: if ignore training no bbox img, rewrite load_file_list
    '''
    __shared__ = ['generate_anchors_fn', 'preprocess_fn', 'out_bbox_anchor']
    ...
    def __init__(self,
                phase='training',
                debug_img_dir='./debug_img_dir',    # to update
                base_data_path = './dataset',   # to update
                gt_file='WIDERFACE/wider_face_split/wider_face_train_bbx_gt.txt',   # to update
                img_dir_name='WIDERFACE/WIDER_train/images',    # to update
                generate_anchors_fn=None,
                anchor_target_fn=None,
                preprocess_fn=None,
                data_aug_settings=None,
                ...
                gt_list=None,
                out_dsfd_label=False):
        WiderFaceTrainSet.__init__(self, phase, debug_img_dir, base_data_path, gt_file, img_dir_name, generate_anchors_fn, anchor_target_fn, preprocess_fn, data_aug_settings, out_bbox_anchor, gt_list, out_dsfd_label)