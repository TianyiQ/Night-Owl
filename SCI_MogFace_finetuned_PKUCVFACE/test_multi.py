...
from data.datasets_utils.widerface import GetFileSuffix

from modelling.architectures.widerface_basenet import WiderFaceBaseNet

parser = argparse.ArgumentParser(description='Test Details')
parser.add_argument('--num_iter', '-n', default=None, type=int, help='number of iteration for test.')
...
parser.add_argument('--config', '-c', default='configs/mogface/SCI_MogFace_finetune.yml', type=str, help='config yml.')
...
parser.add_argument('--sci_weights', '-s', default=None, type=str, help='Path to pretrained SCI weights')
parser.add_argument('--final_testing', '-f', default=False, type=bool, help='Whether doing final testing')
parser.add_argument('--final_testing_dir', '-d', default=None, type=str, help='Where to save final testing results')
...
def detect_face(image, shrink):
    ...
    x = net.enhance(x)
    torch.cuda.empty_cache()
    out = WiderFaceBaseNet.forward(net, x)
    torch.cuda.empty_cache()
...
def write_to_txt_DF(f, det, height, width, img_name, img_dir_name):
    # f.write('{:s}\n'.format(img_dir_name + '/' + img_name + '.png'))
    # f.write('{:d}\n'.format(det.shape[0]))
...
def write_to_txt(f, det, height, width, img_name, img_dir_name):
    f.write('{:s}\n'.format(img_dir_name + '/' + img_name + GetFileSuffix.get_suf(img_name)))
    f.write('{:d}\n'.format(det.shape[0]))
    for i in range(det.shape[0]):
        if det[i][0] < 0.0:
            xmin = 0.0
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        assert args.final_testing is not None
        assert args.num_iter is not None
        if args.sci_weights is None:
            print('Not using pretrained SCI', end='\n\n')
        if args.test_hard:
        ...
        net.load_state_dict(torch.load(model_name), sci=args.sci_weights)
        net.cuda()
        net.eval()
        ...
        for (img, img_name, img_dir_name) in tqdm(val_set_iter):
            ...
            save_img_name = os.path.join((args.final_testing_dir if args.final_testing else event_dir), img_name + '.txt')
            with open(save_img_name, 'w') as f:
                if args.final_testing:
                    assert args.final_testing_dir is not None
                    write_to_txt_DF(f, dets, img.shape[0], img.shape[1], img_name, img_dir_name)
                else:
                    write_to_txt(f, dets, img.shape[0], img.shape[1], img_name, img_dir_name)
        
        if not args.final_testing:
            pred = abs_save_dir
            gt_path = './dataset/WIDERFACE/ground_truth'
            test_iter = args.num_iter
            det_result_txt = det_result_txt

            from evaluation.eval_map import eval_map
            easy_map, medium_map, hard_map = eval_map(pred, gt_path)
            print('[mAP] Test Iter: {}, Easy: {}, Medium: {}, Hard: {}'.format(test_iter, easy_map, medium_map, hard_map))
            ...
            print('[AP50] Test Iter: {}, Easy: {}, Medium: {}, Hard: {}'.format(test_iter, easy_ap, medium_ap, hard_ap))
