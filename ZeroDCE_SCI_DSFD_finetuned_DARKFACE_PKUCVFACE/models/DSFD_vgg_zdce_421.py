...
class DSFD(nn.Module):
    ...
    def expand_conv(self):
        if self.expanded:
            return
        self.expanded = True
        with torch.no_grad():
            tmp_ = self.vgg[0].weight
            tmp = nn.Conv2d(tmp_.shape[1]*4,tmp_.shape[0],(tmp_.shape[2],tmp_.shape[3]),padding=1,bias=True)
            for i in range(4):
                tmp.weight[:,tmp_.shape[1]*i:tmp_.shape[1]*(i+1),:,:] = tmp_/4
            tmp.bias = self.vgg[0].bias
            self.vgg[0] = tmp
    
    def contract_conv(self):
        if not self.expanded:
            return
        self.expanded = False
        with torch.no_grad():
            self.vgg[0] = nn.Conv2d(3,64,3,padding=1)
...
class DSFD_zdce(nn.Module):
    ...
    def load_weights_dsfd(self, base_file):
        try:
            ...
            self.dsfd.contract_conv()
            self.dsfd.load_weights(base_file)
            self.dsfd.expand_conv()
