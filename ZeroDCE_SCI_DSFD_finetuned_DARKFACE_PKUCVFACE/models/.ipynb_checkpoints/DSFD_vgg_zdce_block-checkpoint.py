...
def light(gr):
    # g : shape [1, 3, N, M]
    gr = torch.minimum(torch.maximum(gr, torch.tensor(0.001)),torch.tensor(0.999))
    _pow = torch.zeros(gr.shape, requires_grad = False)
    assert False not in (gr>=0)
    assert False not in (gr<=1)
    with torch.no_grad():
        for ID in range(gr.shape[0]):
            g = gr[ID]
            alpha = 0.5
            n, m = g.shape[1], g.shape[2]
            a, b = 4, 6
            for i in range(a):
                for j in range(b):
                    gg = g[:, i * n // a : (i + 1) * n // a, j * m // b : (j + 1) * m // b]
                    su = gg.shape[1] * gg.shape[2] * 0.7
                    alpha = torch.tensor(0.5)
                    logg = torch.log(gg + 1e-5)
                    for Iter in range(4):
                        power_g = torch.pow(gg, alpha)
                        alpha -= (torch.sum(power_g) - su) / torch.sum(power_g * logg)
                        alpha = torch.max(alpha, torch.tensor(0.1))
                        #print('debug : ', alpha)
                    #print('test : ', alpha)
                    _pow[ID, :, i * n // a : (i + 1) * n // a, j * m // b : (j + 1) * m // b] = alpha.detach()
    #                 print('debug : ', alpha.detach())
    #                 g[:, i * n // a : (i + 1) * n // a, j * m // b : (j + 1) * m // b] = torch.pow(g[:, i * n // a : (i + 1) * n // a, j * m // b : (j + 1) * m // b], alpha)
    #         print('debug : ', _pow[0][0][0])
    assert True not in torch.isnan(_pow)
    gr = gr ** _pow.detach()
    assert True not in torch.isnan(gr)
    assert False not in (gr>=0)
    assert False not in (gr<=1)
#     print("debug _pow: ",torch.min(_pow),torch.max(_pow))
    return gr
...
class DSFD_zdce(nn.Module):
    ...
    def forward(self,x):
        ...
        """
        # return self.dsfd(x)
        # print(x.shape,x[0][0][0][:10])
        assert True not in torch.isnan(x)
        tmp=cfg.img_mean_rgb.clone()
        if torch.cuda.is_available():
            tmp=tmp.cuda()
        with torch.no_grad():
            x=(x+tmp)/255.
            assert False not in (x>=0)
            assert False not in (x<=1)
            # t1 = time.time()
            _,x,_=self.zdce(x)
            x = light(x)
            self.enhanced_image = x
#         _, self.final_image, _ = self.zdce(x)
        # t2 = time.time()
        # print("ZeroDCE: {}",t2-t1)
        x=x*255-tmp
        # print(x.shape,x[0][0][0][:10])
    ...
    def load_weights_dsfd(self, base_file):
        return self.dsfd.load_weights(base_file)
    
    def load_weights_zdce(self, base_file):
        return self.zdce.load_state_dict(torch.load(base_file,map_location=torch.device('cpu')))
