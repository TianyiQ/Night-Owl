...
class DSFD_sci(nn.Module):
    """
    DSFD with a SCI in front
    """
    def __init__(self, phase, base, extras, fem, head1, head2, num_classes):
        super().__init__()
        self.sci=Finetunemodel()
        self.dsfd=DSFD(phase, base, extras, fem, head1, head2, num_classes)
    ...
    def load_weights_sci(self, base_file):
        return self.sci.load_weights(base_file)
...
def build_net_vgg_sci(phase, num_classes=2):
    base = vgg(vgg_cfg, 3)
    extras = add_extras(extras_cfg, 1024)
    head1 = multibox(base, extras, num_classes)
    head2 = multibox(base, extras, num_classes)
    fem = fem_module(fem_cfg)
    return DSFD_sci(phase, base, extras, fem, head1, head2, num_classes)