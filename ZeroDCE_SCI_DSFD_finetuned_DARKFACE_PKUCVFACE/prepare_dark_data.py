...
parser.add_argument('--valnum',
                    default=-1, type=int,
                    help='the number of fold to validate on')
args = parser.parse_args()

if args.valnum not in list(range(10)):
    print('specify a valid fold')
    exit()

WIDER_ROOT = '/code/data/WIDER'
WIDER_TRAIN = os.path.join(WIDER_ROOT, 'WIDER_train', 'images')
WIDER_VAL = os.path.join(WIDER_ROOT, 'WIDER_val', 'images')
train_list_file = os.path.join(WIDER_TRAIN, f'{args.valnum}--Whatever',
                               'train.txt')
val_list_file = os.path.join(WIDER_TRAIN, f'{args.valnum}--Whatever',
                               'val.txt')
