...
from SCI.model import EnhanceNetwork as SciEnhance
from SCI.model import Finetunemodel as SciFinetune

@register
class SciMogfaceNet(WiderFaceBaseNet):
    __shared__ = ['phase']
    __inject__ = ['backbone', 'fpn', 'pred_net']
    def __init__(self, backbone, fpn, pred_net, phase='training', out_bb_ft=False):
        WiderFaceBaseNet.__init__(self, backbone, fpn, pred_net, phase, out_bb_ft)
        self.enhance = SciEnhance(layers=1, channels=3) # 训练和测试都采用单层，也不使用self-calibration
    ...
    def forward(self, x):
        ...
        x = self.enhance(x)
        # x = torch.flip(x, [1])
        return WiderFaceBaseNet.forward(self, x)
    
    def load_state_dict(self, state_dict, sci):
        WiderFaceBaseNet.load_state_dict(self, state_dict, strict=False)
        if sci is not None:
            assert isinstance(sci, str) and sci
            temp_model = SciFinetune(sci)
            self.enhance.load_state_dict(temp_model.enhance.state_dict())
